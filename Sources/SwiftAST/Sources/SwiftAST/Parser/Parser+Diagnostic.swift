...
public enum ParserErrorKind : DiagnosticKind {
  ...
  case noTypeNamedInModule(String,String)
  case internalError(String)
  case cannotSpecializeNonGenericType(String)
  case missingSpecializationArguments(String)
  case illegalNumberOfSpecializationArguments(String)
  case cannotSpecializeNonGenericFunc(String)
  case missingSpecializationFuncArguments(String)
  case illegalNumberOfFuncSpecializationArguments(String)
  case noSuchModule(String)
  case invalidRedeclaration(String)
  case expressionsAtTopLevel
  case inaccessibleElement(String,String)
  case cannotAssign(String)
  case cannotFind(String)
  case noAccessibleInitializers(String)
  case ambiguiousInitializer(String)
  case ambiguiousFunction(String)
  ...
  public var diagnosticMessage: String {
    ...
    case .noTypeNamed(let name):
      return "cannot find type '\(name)' in scope"
    case .noTypeNamedInModule(let name, let module):
      return "no typed named '\(name)' in module '\(module)'"
    case .internalError(let msg):
      return "internal error: \(msg)"
    case .cannotSpecializeNonGenericType(let name):
      return "cannot specialize non-generic type '\(name)'"
    case .missingSpecializationArguments(let name):
      return "missing specialization arguments for generic type '\(name)'"
    case .illegalNumberOfSpecializationArguments(let name):
      return "illegal number of specialization arguments for generic type '\(name)'"
    case .cannotSpecializeNonGenericFunc(let name):
      return "cannot specialize non-generic function '\(name)'"
    case .missingSpecializationFuncArguments(let name):
      return "missing specialization arguments for generic function '\(name)'"
    case .illegalNumberOfFuncSpecializationArguments(let name):
      return "illegal number of specialization arguments for generic function '\(name)'"
    case .noSuchModule(let name):
      return "no such module '\(name)'"
    case .invalidRedeclaration(let name):
      return "invalid redeclaration of '\(name)'"
    case .expressionsAtTopLevel:
      return "expressions are not allowed at the top level"
    case .inaccessibleElement(let id,let level):
      return "'\(id)' is inaccessible due to '\(level)' protection level"
    case .cannotAssign(let id):
      return "cannot assign to value: '\(id)' is a 'let' constant"
    case .cannotFind(let id):
      return "cannot find '\(id)' in scope"
    case .noAccessibleInitializers(let id):
	  return "type '\(id)' has no accessible initializers"
    case .ambiguiousInitializer(let id):
	  return "initializer call to '\(id)' is ambiguous"
	case .ambiguiousFunction(let id):
	  return "call to '\(id)' is ambiguous"
